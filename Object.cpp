...
float Object::cy = 0.0;
float Object::cz = 0.0;
int Object::rCameraX = 50;
int Object::rCameraY = 20;
float Object::txcamera = -0.28;
float Object::tycamera = -0.03;
float Object::tzcamera = -1.0;
...
Object::Object(float betx, float bety, float betz, int berx, int bery, int berz, float besx, float besy, float besz, bool fenyek, char* betexture, int bemozaik, char* beobjname, int beapp, int beflag)
{
    ...
    {
        char temp[32];
        new_textureid.texture = new GLuint[47];
        new_textureid.tex_num_per_obj = 47;
        glGenTextures(47, new_textureid.texture);
        for (int i=0; i<26; i++)
        {
            strcpy(temp, texname);
            strcat(temp, "/");
...
Object::~Object()
{
    if (strncmp(objname, "negyzet", 7))
    {
        ...
        if ((*mesh_id).mesh_numbers == 0)
        {
            ...
            delete (*mesh_id).mesh;
            for (int i=0; i<(*mesh_id).mesh_num_per_obj; i++)
                delete [] (*mesh_id).texcoords[i];
            delete [] (*mesh_id).texcoord_numbers;
    ...
    if (strcmp(texname, "NULL"))
    {
        ...
        {
            glDeleteTextures((*textureid).tex_num_per_obj, (*textureid).texture);
...
void Object::rendering(const int & i)
{
    ...
    if (appear < 2)
    {
        //glRotatef(50, 1, 0, 0); //alap elforgatas a kameranezetnek megfeleloen
        //glRotatef(20, 0, 1, 0);

        //az elem elforgatasa a kamerahoz kepest

        glTranslatef(txcamera, tycamera, tzcamera); //a kulso nezet miatti eltolas (belso nezet eseten ez nem kell)

        glRotatef(rCameraX, 1, 0, 0);
        glRotatef(rCameraY, 0, 1, 0);
        ...
        //az elem elforgatasa az abszolut helyzetetehez kepest
        glRotatef(rx, 1, 0, 0);
        glRotatef(ry, 0, 1, 0);
        glRotatef(rz, 0, 0, 1);
    ...
    while (title[i] != 0)
    {
        if (title[i] != ' ' && title[i] != '_')
        {
            ...
            if (appear == 5)
            {
                ...
                glTranslatef(tx+(i-j)*sx*1.2, ty, tz+i*(float)rz/10000.0); //eltolas az abszolut helyre
            ...
            {
                glTranslatef(tx+(i-j)*sx*0.6, ty, -0.25);
                glRotatef(-90, 1, 0, 0);
                glRotatef(90, 0, 1, 0);
                glScalef(sx, sy, sz);
            }
